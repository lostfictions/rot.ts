...
interface LightingMap { [position: string]: [number, number, number] }
...
export class Lighting {
   ...
   */
  constructor(reflectivityCallback: ReflectivityCallback, options?: Partial<LightingOptions>) {
  ...
  compute(lightingCallback: LightingCallback): this {
    ...
    for (const [pos, light] of Object.entries(this._lights)) { /* prepare emitters for first pass */
    ...
    for (const [pos, color] of Object.entries(litCells)) { /* let the user know what and how is lit */
   ...
   */
  private _emitLight(emittingCells: LightingMap, litCells: LightingMap, doneCells: Set<string>): void {
   ...
   * @param doneCells
   */
  private _computeEmitters(litCells: LightingMap, doneCells: Set<string>): LightingMap {
    const result: LightingMap = {};

    for (const [pos, color] of Object.entries(litCells)) {
      if(doneCells.has(pos)) { continue; } /* already emitted */
      ...
      if (intensity > this._options.emissionThreshold) { result[pos] = emission as [number, number, number]; }
    }
   ...
   */
  private _emitLightFromCell(x: number, y: number, color: [number, number, number], litCells: LightingMap): void {
    const key = x + "," + y;
